...
{
    public class TempUDPSecurity : Interfaces.ISecurity
    {
        ...
        private TransferBuffer FormatPacket(Packet packet)
        {
            ...
            {
                pWriter.Write(packet.ReadUInt8Array(2));
                pWriter.Write((byte)0x00);
                pWriter.Write((ushort)(bytes.Length - 2));
                pWriter.Write(packet.Opcode1);
                pWriter.Write(packet.Opcode2);
                pWriter.Write((ushort)(bytes.Length - 2 + 7));
                pWriter.Write((byte)0x00); //USUALLY 0, UNKNOWN CURRENTLY
                pWriter.Write(bytes.Skip(2).ToArray());
